...
- `npm test`：✅ **172 测试全绿**（经多轮扩展：camera/difficulty/highscore/tutorial/specials/world/server 等）
- `npm run lint`：✅ 零错误零警告
...
- ~~**P2 extendLine 头部插入未调 t**~~ → **已完成**（simulation.ts:348-361 按段长度比例重映射 t）
- ~~**P3 ResizeObserver 自适应**~~ → **已完成**（世界扩展为 3840×1200，摄像机系统支持缩放/平移，首帧 fit + resize 监听）
